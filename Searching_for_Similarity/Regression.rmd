...
set.seed(1234)
train_index <- sample(1:nrow(unpopular_songs), size = 0.8*nrow(unpopular_songs), replace = FALSE) # nolint
...
```{r}
lm1 <- lm(popularity ~ danceability+energy+key+key+loudness+speechiness+acousticness+liveness+valence+tempo+duration_ms, data = train) # nolint
summary(lm1)
par(mfrow=c(1,1))
